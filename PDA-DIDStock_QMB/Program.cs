...
namespace PDA_DIDSlotLink
{
    static class Program
    {
        ...
        static void Main()
        {
            string text;
            string text2;
            //text2 = "Server=10.97.2.11;database=SMT;uid=sa;pwd=pqmb#7sa;Connect Timeout=30";
            text2 = "Server=10.18.11.11;database=SMT;uid=sa;pwd=East#86;Connect Timeout=30";
            text = EncryptStr.EncryptDES(text2, "1234ABCD");
            text = EncryptStr.DecryptDES(text, "1234ABCD");
            Parameter.strUserRight = "Login";
            frmLogin frmlogin = new frmLogin();
            if (frmlogin.ShowDialog() != DialogResult.OK)
            {
                return;
            }
            Application.Run(new frmDIDStock());
